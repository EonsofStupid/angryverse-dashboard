...
import { useAuthStore } from "@/store/useAuthStore"; // No more useRoleCheck import
...
export const ThemeProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  // Pull the user's role from the auth store
  ...
  /**
   * Applies theme variables to the document root (CSS custom properties).
   */
  const applyThemeVariables = useCallback(() => {
    if (currentTheme?.configuration) {
      ...
      if (effects) {
        ...
        if (effects.glass) {
          ...
          if (shadow_composition) {
            Object.entries(shadow_composition).forEach(([key, val]) => {
              root.style.setProperty(`--glass-shadow-${key}`, val.toString());
            });
          }
        }
        ...
        if (effects.animations) {
          Object.entries(effects.animations.timing).forEach(([key, val]) => {
            root.style.setProperty(`--animation-timing-${key}`, val);
          });
          Object.entries(effects.animations.curves).forEach(([key, val]) => {
            root.style.setProperty(`--animation-curve-${key}`, val);
          });
        }
      ...
      // 3. Set some semantic color variables
  ...
  /**
   * Initialize theme on first render if not already initialized.
   */
  useEffect(() => {
    if (!isInitialized) {
  ...
  /**
   * Whenever theme changes, apply it to CSS vars.
   */
  useEffect(() => {
    if (currentTheme?.configuration) {
  ...
  /**
   * Fetch page-specific theme or fallback to the default theme.
   */
  const initializeTheme = useCallback(async () => {
    try {
      ...
      if (pageTheme?.themes) {
        const themeData = pageTheme.themes;
      ...
      // No page-specific theme => load default theme
  ...
  /**
   * Provide theme and auth-derived info (isAdmin) to children
   */
  const value = {
    currentTheme,
    isLoading,     // from the theme store
    error,
    setCurrentTheme,
    fetchPageTheme,
    applyThemeVariables,
  ...
  return (
    <ThemeContext.Provider value={value}>
      ...
      {/* If theme store is loading, show a small overlay */}
      {isLoading && (
